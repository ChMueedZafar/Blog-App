...
export const createBlog = async (req, res) => {
  try {
    ...
    // Log the files to see what we're receiving
    console.log("Received files:", req.files);

    // Check what's in the files object
    const fileKeys = Object.keys(req.files);
    const blogImage = req.files[fileKeys[0]];  // Get the first file regardless of field name

    if (!blogImage) {
      return res.status(400).json({ message: "Invalid blog image upload" });
    }
